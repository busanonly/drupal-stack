...
# Drush:
drush theme:enable __THEME__ -y
...
```
__THEME__/
├── __THEME__.info.yml         # metadata, region, library global
├── __THEME__.libraries.yml    # definisi CSS/JS (library `base`)
├── __THEME__.theme            # hook PHP (preprocess, suggestions, form settings)
...
1. **Region** — deklarasi di `__THEME__.info.yml` (`regions:`), render di
...
2. **CSS/JS** — letakkan berkas di `css/` / `js/`, daftarkan sebagai library di
   `__THEME__.libraries.yml`, lalu pasang global (`libraries:` di info.yml) atau
   per-template (`{{ attach_library('__THEME__/nama_library') }}`).
...
4. **Logika PHP** — tulis di `__THEME__.theme` (di-preprocess, saran template,
...
- Ingin memakai theme core sebagai basis (mis. Claro/Olivero)? Set
  `base theme: olivero` di `__THEME__.info.yml`, atau pakai generator bawaan core:
